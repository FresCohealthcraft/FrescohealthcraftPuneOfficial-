...
export default function MenuGrid({ 
  ...
  return (
    <section id="menu" className="py-20 bg-gradient-to-b from-[#F2FAED] via-white to-[#FCFDFC] scroll-mt-20 border-t border-[#1A1A1A]/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            return (
              <button
                ...
                className={`py-2 px-5 text-xs font-bold uppercase tracking-wider cursor-pointer rounded-full transition-all duration-300 flex items-center space-x-1.5 shadow-sm active:scale-98 ${
                  isActive
                    ? "bg-[#38A325] text-white shadow-none"
        ...
        {!selectedCategory && !searchTerm && (
          ...
          >
            ...
            <div className="absolute -top-24 -left-24 w-48 h-48 rounded-full bg-[#38A325]/5 blur-3xl pointer-events-none" />
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item, index) => (
              <motion.div
                ...
                key={item.id}
                className="bg-white border border-[#1A1A1A]/10 rounded-2xl p-2.5 sm:p-5.5 shadow-[0_4px_24px_rgba(0,0,0,0.01)] hover:border-[#38A325]/30 transition-all duration-300 flex flex-col justify-between h-full text-left group"
              >
                <div>
                  ...
                  <div className="relative w-full aspect-video rounded-xl sm:rounded-2.5xl bg-[#EFECE5] select-none flex items-center justify-center text-3.5xl sm:text-5.5xl mb-2 sm:mb-5 overflow-hidden transition-transform group-hover:scale-[1.01] duration-300">
                    ...
                    <span className="absolute bottom-1 left-1 text-[8px] sm:text-[10px] font-bold text-[#38A325] bg-white/90 border border-[#1A1A1A]/5 px-1 sm:px-2 py-0.5 rounded-md flex items-center gap-1 scale-90 uppercase tracking-wider">
                      <Leaf className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-[#38A325]" />
                    ...
                    <button
                      ...
                      className="absolute bottom-1 right-1 p-1 sm:p-2 bg-white hover:bg-[#38A325] hover:text-[#F9F8F4] text-[#38A325] rounded-full border border-[#1A1A1A]/10 transition-colors cursor-pointer shadow-sm z-10"
                  ...
                  {/* Title & Description */}
                  <h3 className="text-xs sm:text-base font-bold text-[#1A1A1A] group-hover:text-[#38A325] transition-colors duration-200">
                    {item.name}
                  </h3>
                ...
                <div className="mt-2.5 sm:mt-5 pt-2.5 sm:pt-4 border-t border-[#1A1A1A]/5 flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[13px] sm:text-2xl font-bold font-serif text-[#38A325] leading-none">
                      ₹{item.price}
                    </span>
                  ...
                  <button
                    ...
                    className="bg-[#1A1A1A] hover:bg-[#38A325] text-white px-2 sm:px-4 py-1 sm:py-2 rounded-full text-[8.5px] sm:text-[10px] font-bold uppercase tracking-wider flex items-center space-x-1 cursor-pointer transition-colors shrink-0"
                  >
