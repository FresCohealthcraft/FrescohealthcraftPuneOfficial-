...
export default function SubscriptionSection({ onAddToCartDirectly }: SubscriptionSectionProps) {
  ...
  const monthlySubscriptions = [
    {
      ...
      name: "Cleanse Green Taster",
      icon: "🥝",
      subtitle: "8-Day Weekend Detox Cycle",
      deliveries: "8 deliveries / month (Saturdays & Sundays)",
      savings: "Save ₹310 off standard menu",
      price: 999,
      benefits: [
        "1 Fresh organic natural Detox Juice daily",
        "Complimentary superfood protein chia seeds setup",
    ...
    },
    {
      ...
      name: "Standard Balanced Cleanse",
      icon: "🥑",
      subtitle: "15-Day Alternate Day Cycle",
      deliveries: "15 deliveries / month (alternate days schedule)",
      savings: "Save ₹750 off standard menu",
      price: 1999,
      benefits: [
        "Alternating fruits, veggies & premium nutrition smoothies",
        "Free dietary consulting sessions with our head Pune nutritionist",
        "Priority 30-min morning delivery slot guarantee",
        "Exclude ingredients (e.g. sugar, dairy) on command",
        "Pause subscription anytime via our portal/WhatsApp"
      ],
      popular: true,
      bgColor: "bg-gradient-to-b from-white to-[#4A8824]/5",
      accentColor: "#4A8824",
  ...
  return (
    <section id="subscriptions" className="py-20 bg-[#F9F8F4] scroll-mt-20 border-t border-[#1A1A1A]/10 relative overflow-hidden">
      
      {/* Background decorations */}
      <div className="absolute right-0 top-1/4 w-96 h-96 bg-[#4A8824]/5 rounded-full filter blur-3xl -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="bg-[#EFECE5] p-1.5 rounded-full inline-flex space-x-1.5 border border-[#1A1A1A]/5 shadow-inner">
            <button
              ...
              className={`py-3 px-6 sm:px-8 rounded-full text-xs font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center space-x-2 ${
                activeTab === "weekly"
                  ? "bg-[#4A8824] text-white shadow-md"
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center max-w-4xl mx-auto mb-10 p-5 bg-[#F9F8F4] border border-[#1A1A1A]/10 rounded-2.5xl">
                <div className="flex items-center space-x-2.5 text-left">
                  ...
                  <div>
                    <h4 className="font-bold text-xs uppercase tracking-widest text-[#4A8824]">
                ...
                {weeklyPlans.map((plan) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[#4A8824] font-bold text-xs uppercase tracking-wide mt-1">
                        {plan.subtitle}
                      </p>
                    ...
                    <div className="mt-6 pt-4.5 border-t border-[#1A1A1A]/5 flex items-center justify-between">
                      <div className="flex items-baseline space-x-2">
                        <span className="text-2xl font-serif italic font-bold text-[#4A8824]">
                          ₹{plan.price}
                        </span>
                      ...
                      <button
                        ...
                        className="bg-[#1A1A1A] hover:bg-[#4A8824] hover:scale-105 active:scale-95 text-white p-2.5 rounded-full cursor-pointer transition-all duration-300 shadow-sm flex items-center justify-center"
                ...
                {monthlySubscriptions.map((sub) => (
                  <motion.div
                    ...
                    className={`rounded-3xl p-8 border hover:shadow-xl transition-all duration-300 flex flex-col justify-between relative ${sub.bgColor} ${
                      sub.popular 
                        ? "border-[#4A8824] shadow-md ring-2 ring-[#4A8824]/10" 
                        : "border-[#1A1A1A]/10 shadow-sm"
                    }`}
                  >
                    {/* Popular Badge recommendation */}
                    {sub.popular && (
                      <span className="absolute -top-3.5 right-6 bg-[#4A8824] text-white text-[9.5px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-md">
                        Best Seller
                      </span>
                    )}
                    ...
                    <div>
                      ...
                      <div className="mt-6 p-4 bg-[#EFECE5]/35 rounded-2xl border border-[#1A1A1A]/5 text-center">
                        ...
                        <span className="text-[10px] font-bold text-[#4A8824] bg-[#4A8824]/10 px-2.5 py-0.5 rounded-full inline-block mt-2">
                          {sub.savings}
                        </span>
                      </div>
                      ...
                      <div className="mt-6 space-y-3 font-medium text-xs text-gray-600">
                        ...
                        <p className="text-[#1A1A1A] font-bold flex items-center gap-1.5 bg-[#4A8824]/5 py-2 px-3.5 rounded-xl border border-[#4A8824]/15">
                          <Calendar className="w-4 h-4 text-[#4A8824]" />
                        ...
                        <ul className="space-y-2.5">
                          {sub.benefits.map((b, i) => (
                            <li key={i} className="flex items-start space-x-2.5 text-[#1A1A1A]/85">
                              <CheckCircle2 className="w-4 h-4 text-[#4A8824] shrink-0 mt-0.5" />
                    ...
                    <div className="mt-8 pt-6 border-t border-[#1A1A1A]/5">
                      <button
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center space-x-2 shadow-sm ${
                          sub.popular
                            ? "bg-[#4A8824] hover:bg-[#4A8824]/95 text-white"
                            : "bg-[#1A1A1A] hover:bg-[#4A8824] text-white"
                        }`}
                      >
        ...
        <AnimatePresence>
          {showSubscriptionSuccess && (
            ...
            >
              <div className="p-1 px-1.5 bg-[#4A8824] rounded-full text-white text-xs">✔</div>
              <div>
